...
@pytest.mark.skipif(condition=(os.name == "nt"), reason="Not compatible with Windows")
def test_add_update_existing_client_through_cli():
    # Initializes the app and checks if the main menu screen is reached.
    app = pexpect.spawn("python3 app/main.py")
    app.expect(
        re.compile(r"=== Gerenciador de Faturas ==="),
...
@pytest.mark.skipif(condition=(os.name == "nt"), reason="Not compatible with Windows")
def test_add_and_update_product_through_cli():
    app = pexpect.spawn("python3 app/main.py", encoding='utf-8')
    app.expect(re.compile(r"=== Gerenciador de Faturas ==="), timeout=3)
    ...
    # Update the product and checks if it was removed successfully.
    app.sendline("3") 
    app.sendline(product_id)
    app.sendline("ProdutoTeste2")
    app.sendline("12.345")
    app.expect(
        re.compile(
            r'Produto atualizado: \{"id": ., "name": "ProdutoTeste2", "price": 12.345\}'
        ),
        timeout=3,
    )
    app.sendline("")